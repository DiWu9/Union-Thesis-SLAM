...
class TSDFVolume:
  ...
  def __init__(self, vol_bnds, voxel_size, use_gpu=True):
    ...
    """
    print("Initializing voxel grids ... ")
    ...
    # update volume maximum bound to accord with vol dimension
  ...
  def integrate(self, color_im, depth_im, cam_intr, cam_pose, obs_weight=1.):
    ...
    # np.floor gets the minimum integer greater than the float value
    # convert 3D matrix (640*480*3) to 2D (640*480), use RGB in a single float32
    ...
    if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
      ...
      # multiplicative inverse of camera pose, why inverse?
      cam_pts = rigid_transform(cam_pts, np.linalg.inv(cam_pose))
      
      # maintain z coordinate and use camera pose to convert x,y camera coordinates to x,y pixel coordinates
      pix_z = cam_pts[:, 2]
      ...
      depth_diff = depth_val - pix_z
        # only the points greater than 0 and smaller than truncate margin are valid
      valid_pts = np.logical_and(depth_val > 0, depth_diff >= -self._trunc_margin)
        # why the upper bound of distance is 1?
      ...
      valid_vox_z = self.vox_coords[valid_pts, 2]
        ...
        # get the current weight on the voxels
      w_old = self._weight_vol_cpu[valid_vox_x, valid_vox_y, valid_vox_z]
      tsdf_vals = self._tsdf_vol_cpu[valid_vox_x, valid_vox_y, valid_vox_z]
        # update weight and distance value for each valid voxel
      ...
      # mix new color to the current color value
